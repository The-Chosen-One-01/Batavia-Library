...
def add_new_user():
    ...
    if find_user(username):
        return render_template('sign_up.html',warning=True)
...
def checkout():
    return_date = clock()
    user = session.get('user')
    return render_template('checkout.html',return_date = return_date, user = user)

@app.route('/checkout-successful')
def post_checkout():
    username = session.get('user')
    failed = None
    return render_template('checkout_success.html', user = username, failed = failed)
...
def process_checkout():
    ...
    db = sqlite3.connect(DATABASE)
    cursor = db.cursor()
    query = f"insert into borrow (user_ID, book_ID, genre_ID, address, borrow_date, borrow_due) values ({all_IDs[0]}, {all_IDs[1]}, {all_IDs[2]}, '{address}', {datetime.now().strftime("%d-%m-%Y")}, {clock()});"
    cursor.execute(query)
...
def find_IDs(username, book):
    db = sqlite3.connect(DATABASE)
    cursor = db.cursor()
    cursor.execute(f"select ID, genre_ID from books where book_name like '%{book}%';")
...
def process_return():
    ...
    db = sqlite3.connect(DATABASE)
    cursor = db.cursor()
    query = f"insert into return (user_ID, book_ID, return_date) values ({all_IDs[0]}, {all_IDs[1]},{datetime.now().strftime("%d-%m-%Y")});"
    cursor.execute(query)
    db.commit()
    db.close()
...
def post_return():
    username = session.get('user')
    failed = None
    return render_template('return_successful.html', user = username, failed = failed)
